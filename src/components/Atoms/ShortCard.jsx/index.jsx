...
const ShortCard = (props) => {
    const {Title, Specs, Discount, StartingPrice, PreviousPrice, PhonePic, classname} = props;
    return (
        <div>
            <div className={`flex flex-col bg-[#ffffff] items-center font-inter pt-10 px-5 text-center gap-2 ${classname} md:hover:shadow-lg md:hover:ease-out md:duration-[250ms]" onClick="window.location.href = '/product/#'`}>
                ...
                <div className="flex gap-2 flex-col md:flex md:flex-row md:items-center">
                    <div className="font-normal text-[14px] leading-[18px] md:text-[21px] md:leading-[26px]">Mulai dari Rp {StartingPrice}</div>
                    <div className="font-normal text-[12px] leading-[15px] text-[#999999] line-through md:text-[15px] md:leading-[19px]">{PreviousPrice}</div>
                </div>
                <div className="md:flex md:gap-2 md:my-[33px]">
